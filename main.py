    ...
    try:
        ...
        for link in links:
            ...
            if msg:
                ...
                # Generate AdFly shortened link
                api_token = config["adfly_api_token"]
                long_url = requests.utils.quote(direct_link)
                api_url = f"https://adfly.cc/api?api={api_token}&url={long_url}&alias=CustomAlias&format=text"
                
                try:
                    result = requests.get(api_url)
                    if result.status_code == 200:
                        shortened_link = result.text
